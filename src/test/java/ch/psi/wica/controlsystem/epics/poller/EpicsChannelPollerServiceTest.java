...
import ch.psi.wica.controlsystem.epics.metadata.EpicsChannelMetadataService;
...
class EpicsChannelPollerServiceTest
{
...
/*- Private attributes -------------------------------------------------------*/
   ...
   @Autowired
   private EpicsChannelPollerService epicsChannelPollerService;

   @Autowired
   private EpicsChannelPollerPublisher epicsChannelPollerPublisher;
...
/*- Package-access methods ---------------------------------------------------*/
   ...
   void beforeEach()
   {
      ...
      assertThat( epicsChannelPollerPublisher.getStatistics().getStartRequests(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStopRequests(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getActiveRequests(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollCycleCount(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollSuccessCount(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollFailureCount(), is( "0" ) );
   }

   @AfterEach
   void afterEach()
   {
      epicsChannelPollerService.close();
      assertThat( epicsChannelPollerPublisher.getStatistics().getStartRequests(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStopRequests(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getActiveRequests(), is( "0" ) );
   }
   ...
   {
      assertThrows( NullPointerException.class, () -> epicsChannelPollerService.startPolling( null ) );
   }

   @Test
   void testStartPolling_ThrowsIllegalStateExceptionWhenWicaChannelNotUnique()
   {
      final EpicsChannelPollerRequest pollingRequest1 = createPollRequest( "abcd", 100 );
      final EpicsChannelPollerRequest pollingRequest2 = createPollRequest( "abcd", 100 );
      epicsChannelPollerService.startPolling( pollingRequest1 );

      final var ex = assertThrows( IllegalStateException.class, () -> epicsChannelPollerService.startPolling( pollingRequest2 ) );
   ...
   {
      final EpicsChannelPollerRequest pollingRequest1 = createPollRequest( "abcd", 100 );
      final EpicsChannelPollerRequest pollingRequest2 = createPollRequest( "abcd", 101 );

      epicsChannelPollerService.startPolling( pollingRequest1 );
      assertDoesNotThrow( () -> epicsChannelPollerService.startPolling( pollingRequest2 ) );
   }
   ...
   void testStartPolling_OneHundredOnlineChannelsConnectTime()
   {
      ...
      {
         epicsChannelPollerService.startPolling( createPollRequest( channel, 1000 ) );
      }
   ...
   void testStartPolling_OneHundredOnlineChannelsStatisticsCollection() throws InterruptedException
   {
      ...
      Thread.sleep( 2300 );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStartRequests(), is( "100" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStopRequests(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getActiveRequests(), is( "100" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollCycleCount(), is( "300" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollFailureCount(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollSuccessCount(), is( "300" ) );
   }

   @Test
   void testStopPolling_ThrowsIllegalStateExceptionWhenStoppingPollingChannelThatWasNeverPreviouslyPolled()
   {
      final var ex = assertThrows( IllegalStateException.class, () -> epicsChannelPollerService.stopPolling( createPollRequest("unknown-channel" ) ) );
   ...
   {
      final EpicsChannelPollerRequest pollRequest = createPollRequest( "offline-channel", 100 );

      epicsChannelPollerService.startPolling( pollRequest );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStartRequests(), is( "1" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStopRequests(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getActiveRequests(), is( "1" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollCycleCount(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollSuccessCount(), is( "0" ) );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollFailureCount(), is( "0" ) );
      ...
      epicsChannelPollerService.stopPolling( pollRequest );
   }

   @Test
   void testStopMonitoring_CheckChannelStatisticsAsExpectedWhenDisposingOfflineChannels() throws InterruptedException
   {
      final EpicsChannelPollerRequest pollRequest = createPollRequest( "offline-channel", 500 );
      ...
      epicsChannelPollerService.stopPolling( pollRequest );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStartRequests(), is( "1") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getStopRequests(), is( "1") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getActiveRequests(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollSuccessCount(), is( "0") );
      assertThat( epicsChannelPollerPublisher.getStatistics().getPollFailureCount(), is( "0") );
   }
   ...
   void testStartPolling_CheckStatisticsAsExpectedWhenDealingWithOfflineChannels()
   {
      ...
      assertThat( epicsChannelPollerService.getStatistics().getActiveRequests(), is( "0" ) );

      epicsChannelPollerService.startPolling( createPollRequest("non-existent-channel-1", 100 ) );
      assertThat( epicsChannelPollerService.getStatistics().getActiveRequests(), is( "1" ) );

      epicsChannelPollerService.startPolling( createPollRequest("non-existent-channel-2", 101 ) );
      assertThat( epicsChannelPollerService.getStatistics().getActiveRequests(), is( "2" ) );
   ...
   {
      epicsChannelPollerService.startPolling( createPollRequest("non-existent-channel" ) );
      Thread.sleep( 1000 );
   ...
   void testStartPolling_verifyInitialConnectBehaviour_NotificationSequence() throws InterruptedException
   {
      ...
      epicsChannelPollerService.startPolling( createPollRequest("wica:test:db_ok" ) );
      Thread.sleep( 2500 );
...
/*- Private methods ----------------------------------------------------------*/
   ...
   private EpicsChannelPollerRequest createPollRequest( String name )
   {
      return createPollRequest( name, 4000 );
   }

   private EpicsChannelPollerRequest createPollRequest( String name, int pollingInterval )
   {
      final WicaChannelName wicaChannelName = WicaChannelName.of( name );
      ...
      final WicaChannel wicaChannel = new WicaChannel( wicaChannelName, wicaChannelProperties );
      return new EpicsChannelPollerRequest( wicaChannel );
   }
