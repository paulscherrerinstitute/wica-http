...
docker run --rm -it --name wica-js-container -d paulscherrerinstitute/wica-js:latest
docker pull paulscherrerinstitute/wica-stream:latest
docker pull paulscherrerinstitute/wica-js:latest

docker run --name wica-stream -t --rm \
  -p $TARGET_PORT:80 \
  -p 5062:5062/udp -p 5062:5062/tcp -p 5064:5064/udp -p 5064:5064/tcp -p 5065:5065/udp \
  -v $WEBDIR:/root/static/web \
